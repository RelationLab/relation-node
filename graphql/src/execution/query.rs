...
impl RawQuery {
    ...
    fn convert(self) -> Result<a::SelectionSet, QueryExecutionError> {
        ...
        transform.convert(selection_set)
    }
}
...
impl Transform {
    ...
    /// Interpolate variable references in directives
    ...
    /// Expand fragments and interpolate variables in a field
    fn expand_field(
        &self,
        field: q::Field,
    ...
    fn expand_selection_set(
        ...
        println!("resolve {}", ty.name());
        let types = a::resolve_object_types(&self.schema.schema, ty.name())?
            .into_iter()
            .map(|obj| obj.name().to_string())
            .collect::<Vec<String>>();
        let mut newset = a::SelectionSet::new(span, types);

        for sel in items
            .into_iter()
            .filter(|sel| qast::include_selection(sel) && !qast::skip_selection(sel))
        {
            match sel {
                q::Selection::Field(field) => {
                    let field = self.expand_field(field, ty)?;
                    newset.push(&field);
                }
                q::Selection::FragmentSpread(spread) => {
                    ...
                    let directives = self.interpolate_directives(directives)?;
                    let type_cond =
                        a::TypeCondition::convert(&self.schema.schema, Some(&frag.type_condition))?
                            .intersect(type_cond);
                    let mut selection_set =
                        self.expand_selection_set(frag.selection_set.clone(), &type_cond, ty)?;
                    selection_set.restrict(&type_cond);
                ...
                q::Selection::InlineFragment(frag) => {
                    ...
                    let directives = self.interpolate_directives(directives)?;
                    let type_cond =
                        a::TypeCondition::convert(&self.schema.schema, type_condition.as_ref())?
                            .intersect(type_cond);
                    let mut selection_set =
                        self.expand_selection_set(selection_set, &type_cond, ty)?;
                    selection_set.restrict(&type_cond);
    ...
    fn convert(
        ...
        let root_type = self.schema.document().get_root_query_type().unwrap();
        self.expand_selection_set(selection_set, &a::TypeCondition::Any, root_type.into())
    }
}
